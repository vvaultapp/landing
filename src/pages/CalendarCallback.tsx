...
/**
 * Dedicated Google Calendar OAuth callback page.
 * This page handles the OAuth callback, connects the calendar,
 * then redirects to the Meetings page.
 */
export default function CalendarCallback() {
  ...
  useEffect(() => {
    const handleCallback = async () => {
      try {
        console.log('[CalendarCallback] Processing OAuth callback...');
        ...
        // Prefer exchanging the OAuth code to ensure we capture provider + refresh tokens
        ...
        if (code) {
          ...
          if (error) {
            console.error('[CalendarCallback] Exchange code error:', error);
        ...
        if (!session) {
          ...
          if (sessionError) {
            console.error('[CalendarCallback] Session error:', sessionError);
        ...
        console.log('[CalendarCallback] Session found:', {
        ...
        if (!session.provider_token) {
          console.error('[CalendarCallback] No provider token in session');
          throw new Error('No Google Calendar access token received. Please try connecting again.');
        }

        // Get user's profile for current_workspace_id
        ...
        if (profileError && profileError.code !== 'PGRST116') {
          console.error('[CalendarCallback] Profile error:', profileError);
        ...
        if (!workspaceId) {
          ...
          if (!rpcError && newWorkspaceId) {
            ...
            console.error('[CalendarCallback] Workspace RPC error:', rpcError);
            ...
            if (wsInsertError) {
              console.error('[CalendarCallback] Workspace fallback insert error:', wsInsertError);
            } else if (wsData?.id) {
              ...
              if (memberInsertError && String((memberInsertError as any)?.code || '') !== '23505') {
                console.error('[CalendarCallback] Workspace fallback owner insert error:', memberInsertError);
              } else {
                ...
                if (onboardingInsertError && String((onboardingInsertError as any)?.code || '') !== '23505') {
                  console.warn('[CalendarCallback] Workspace fallback onboarding seed warning:', onboardingInsertError);
                }
                await supabase
                  .from('profiles')
        ...
        if (workspaceId) {
          const { data: membershipRow, error: membershipError } = await supabase
            ...
            .maybeSingle();
          if (membershipError) {
            console.warn('[CalendarCallback] Membership validation warning:', membershipError);
          }
        ...
        // Connect the Google Calendar via edge function
        console.log('[CalendarCallback] Connecting Google Calendar to workspace:', workspaceId);
        console.log('[CalendarCallback] Has refresh token:', !!session.provider_refresh_token);
        ...
        if (!response.ok) {
          console.error('[CalendarCallback] Calendar connect error:', result);
          throw new Error(result.error || 'Failed to connect Google Calendar');
        }

        console.log('[CalendarCallback] Calendar connected successfully:', result);
        setStatus('success');

        if (result.alreadyConnected) {
          toast.info('Google Calendar already connected!');
        } else {
          toast.success('Google Calendar connected successfully!');
        }

        // Always redirect to meetings page
      ...
      <div className="min-h-screen bg-background flex items-center justify-center px-4">
        <div className="text-center max-w-md">
          ...
          <p className="text-muted-foreground mb-6">
            {errorMessage || 'Failed to connect your Google Calendar. Please try again.'}
          </p>
    ...
    <div className="min-h-screen bg-background flex items-center justify-center px-4">
      <div className="text-center">
        ...
        <h1 className="text-xl font-medium mb-2">
          {status === 'success' ? 'Connected!' : 'Connecting Google Calendar...'}
        </h1>
