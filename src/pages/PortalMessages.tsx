...
import { Send, Paperclip, FileText, Image, FileVideo, Download, X, MessageSquare } from '@/components/ui/icons';
import { Button } from '@/components/ui/button';
import { Skeleton } from '@/components/ui/skeleton';
import { Textarea } from '@/components/ui/textarea';
import { supabase } from '@/integrations/supabase/client';
import { ClientMessage, ClientFile, mapDbRowToMessage, mapDbRowToFile } from '@/types/client-portal';
...
export default function PortalMessages() {
  ...
  useEffect(() => {
    ...
    fetchMessages();
  }, [user, authLoading, loading, portalRole, client, navigate, fetchMessages]);
  ...
  return (
    <PortalLayout client={client}>
      <div className="p-8 h-[calc(100vh-0px)] flex flex-col">
        <div className="mb-6">
          <h1 className="text-[36px] font-bold mb-2">Messages</h1>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-6">
            {messagesLoading ? (
              <div className="space-y-4">
                ...
                <div className="flex justify-end">
                  <Skeleton className="h-12 w-[56%] max-w-[460px] rounded-2xl bg-white/[0.08]" />
                </div>
                <div className="flex justify-start">
                  <Skeleton className="h-10 w-[44%] max-w-[360px] rounded-2xl bg-white/[0.06]" />
                </div>
              </div>
            ) : messages.length === 0 ? (
