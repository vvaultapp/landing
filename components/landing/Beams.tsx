...
function extendMaterial(
  ...
  Object.entries(cfg.uniforms ?? {}).forEach(([key, u]) => {
    (uniforms as Record<string, unknown>)[key] =
      u !== null && typeof u === "object" && "value" in (u as Record<string, unknown>) ? u : { value: u };
  });
...
const CanvasWrapper = ({ children, frameloop }: { children: React.ReactNode; frameloop: "always" | "never" }) => (
...
const Beams = ({
  ...
  return (
    <div ref={containerRef}>
      <CanvasWrapper frameloop={frameloop}>
...
const MergedPlanes = forwardRef<THREE.Mesh, MergedPlanesProps>(({ material, width, count, height }, ref) => {
  ...
  useFrame((state, delta) => {
    // Cap delta to prevent large jumps after tab switch or lag spikes
    const clampedDelta = Math.min(delta, 0.1);
    const mat = mesh.current.material as THREE.ShaderMaterial;
    mat.uniforms.time.value += 0.1 * clampedDelta;
    // Ensure the render loop never stops
    state.invalidate();
  });
