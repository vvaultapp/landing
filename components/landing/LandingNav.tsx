...
function MobileMenu({
  ...
  // Don't render the menu's contents until it's first opened, so nothing
  // menu-related is built on the initial page render (matches desktop).
...
export function NavDropdown({
  ...
  // Defer mounting the heavy, dynamically-imported dropdown panel until this
  // item is opened for the first time — so the panel's JS chunk + content
  // never load on the initial page visit, only once the user opens the menu.
  // Stays mounted afterward, so subsequent opens are instant.
  const [hasOpened, setHasOpened] = useState(false);
  useEffect(() => {
    if (open) setHasOpened(true);
  }, [open]);
  useEffect(() => {
    ...
    // The panel now mounts lazily on first open, so its width isn't known
    // synchronously — re-measure when the content loads in and resizes, so
    // the edge-clamp positioning stays correct.
  ...
  if (!hasChildren) {
    ...
    return (
      <a
        ...
        className="cursor-pointer whitespace-nowrap rounded-xl px-3 py-1.5 text-[14px] font-medium text-white transition-colors duration-200 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/35 lg:px-4 lg:py-2 lg:text-[16px]"
      >
        {item.label}
      </a>
    );
  ...
  return (
    ...
    >
      <button
        type="button"
        onClick={onClick}
        className={`group flex cursor-default items-center gap-1.5 whitespace-nowrap rounded-xl px-3 py-1.5 text-[14px] font-medium text-white transition-colors duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/35 lg:gap-2 lg:px-4 lg:py-2 lg:text-[16px]`}
      >
        {item.label}
        <svg
          viewBox="0 0 12 12"
      ...
      >
        <div
          ...
          style={{
            /* Solid dark panel, no outline — matches the outline-free
               menu tab bar so the whole menu reads as one solid surface. */
            background: "#18181b",
            opacity: open ? 1 : 0,
            transform: open
