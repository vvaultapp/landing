...
export default function CampaignsDocPage() {
  ...
  return (
    <>
      ...
      {/* Email Sequences */}
      <h2 id="email-sequences" className="text-lg font-semibold text-[#111] mt-12 mb-3">
        {locale === "fr" ? "Séquences email" : "Email Sequences"}
      </h2>
      <p className="text-[14px] leading-relaxed text-[#666] mb-4">
        {locale === "fr"
          ? "Crée des campagnes automatisées multi-étapes qui envoient une série d'emails à tes contacts selon un calendrier, avec des analytics par étape et un suivi des destinataires."
          : "Create multi-step automated campaigns that send a series of emails to contacts on a schedule, with per-step analytics and recipient tracking."}
      </p>
      <ul className="text-[14px] text-[#666] mb-4 list-disc pl-5 space-y-1">
        <li><strong className="text-[#444]">{locale === "fr" ? "Jusqu'à 30 étapes" : "Up to 30 steps"}</strong> — {locale === "fr" ? "Configure le sujet, le corps et les packs/tracks joints par étape." : "Configure subject, body, and attached packs/tracks per step."}</li>
        <li><strong className="text-[#444]">{locale === "fr" ? "Délais configurables" : "Configurable delays"}</strong> — {locale === "fr" ? "Définis le délai en jours entre chaque étape." : "Set delay in days between each step."}</li>
        <li><strong className="text-[#444]">{locale === "fr" ? "Inscription par segment" : "Segment enrollment"}</strong> — {locale === "fr" ? "Inscris des contacts individuellement ou par segment." : "Enroll contacts individually or by segment."}</li>
        <li><strong className="text-[#444]">{locale === "fr" ? "Analytics de funnel" : "Funnel analytics"}</strong> — {locale === "fr" ? "Visualisation du funnel, métriques par étape, suivi de la progression par destinataire." : "Funnel visualization, per-step metrics, per-recipient progress tracking."}</li>
        <li><strong className="text-[#444]">{locale === "fr" ? "Cycle de vie" : "Lifecycle"}</strong> — {locale === "fr" ? "Brouillon → Actif → En pause → Terminé. Active, mets en pause et reprends à tout moment." : "Draft → Active → Paused → Completed. Activate, pause, and resume at any time."}</li>
      </ul>
    </>
  );
}
