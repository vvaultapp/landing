    ...
    <main className="min-h-screen bg-[#030305] text-white">
      <div className="mx-auto w-full max-w-4xl px-5 py-14 sm:py-18">
        <div className="mb-10 flex flex-wrap items-center justify-between gap-3">
          <Link
            href="/"
            className="text-xs font-semibold uppercase tracking-[0.18em] text-white/75 hover:text-white"
          >
            vvault
          </Link>
        ...
        <header className="rounded-3xl border border-white/10 bg-white/[0.04] p-6 sm:p-8">
          ...
          <p className="mt-3 max-w-3xl text-sm leading-relaxed text-white/70 sm:text-base">
            vvault provides a workspace to upload and manage audio files, create
            packs and series, share links, send campaigns, track engagement, sell
            licenses, and manage billing. This policy explains what data we
            collect and how we use it to operate and secure the service.
          </p>
...
function LegalSection(props: { title: string; paragraphs: string[] }) {
  return (
    <div>
      <h2 className="text-lg font-semibold text-white">{props.title}</h2>
      <div className="mt-2 space-y-2">
        {props.paragraphs.map((p) => (
          <p key={p} className="text-sm leading-relaxed text-white/70 sm:text-[15px]">
            {p}
          </p>
        ))}
      </div>
